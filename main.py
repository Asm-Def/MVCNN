...
parser.add_argument("-train_path", type=str, default="G:/JasonWu/ZJU/MVCNN/Render2/data/train")
parser.add_argument("-val_path", type=str, default="G:/JasonWu/ZJU/MVCNN/Render2/data/test")
...
def create_folder(log_dir):
    ...
    if not os.path.exists('log'):
        os.mkdir('log')
    if not os.path.exists(os.path.join('log', log_dir)):
        os.mkdir(os.path.join('log', log_dir))
    else:
        input('WARNING: summary folder already exists!! It will be overwritten!!')
        shutil.rmtree(os.path.join('log', log_dir))
        os.mkdir(os.path.join('log', log_dir))
...
if __name__ == '__main__':
    ...
    config_f = open(os.path.join('log', log_dir, 'config.json'), 'w')
    ...
    trainer = Trainer(cnet, train_loader, val_loader, optimizer, nn.CrossEntropyLoss(), 'svcnn', log_dir, num_views=1)
    trainer.train(30)
    ...
    print('num_train_files: '+str(len(train_dataset.filepaths)))
    print('num_val_files: '+str(len(val_dataset.filepaths)))
    trainer = Trainer(cnet_2, train_loader, val_loader, optimizer, nn.CrossEntropyLoss(), 'mvcnn', log_dir, num_views=args.num_views)
    trainer.train(30)
